...
# rnn_inst = HVACOptimizer(a, 96, 100, HVAC)
# rnn_inst.Optimize(2000)

# sess = tf.Session()
# initial_a = tf.truncated_normal(shape=[100, 120, 2], mean=0.0, stddev=0.005).eval(session=sess)
# a = tf.Variable(initial_a, name="action")
# rnn_inst = NAVOptimizer(a, 120, 100, NAVI_BILINEAR)
...
for config in CONFIGURATIONS:
    for step in config['step']:
        name = config['optimizer'].__name__ + ' ' + config['domain'].__name__ + ' Planning Step:' + str(step)
        print name
        ...
        a = tf.Variable(initial_a, name="action")
        rnn_inst = config['optimizer'](a, step, config['batch'], config['domain'])
